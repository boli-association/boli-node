...
import { Params } from "./module/types/proposals/params";
import { Poll } from "./module/types/proposals/poll";
import { Vote } from "./module/types/proposals/vote";
...
export default {
    ...
    actions: {
        init({ dispatch, rootGetters }) {
            console.log('Vuex module: bolimoney.bolinode.proposals initialized!');
        ...
        async QueryVoteAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryVoteAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
