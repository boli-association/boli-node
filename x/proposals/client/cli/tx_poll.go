...
func CmdCreatePoll() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			msg := types.NewMsgCreatePoll(clientCtx.GetFromAddress().String(), argTitle, argsOptions)
...
func CmdUpdatePoll() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			msg := types.NewMsgUpdatePoll(clientCtx.GetFromAddress().String(), id, argTitle, argsOptions)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
